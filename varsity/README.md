...
```bash
cd plank
python3 serve.py varsity
# open http://localhost:8000/varsity/
```
...
node tools/shot.mjs "/varsity/?gallery" g.png --wait 2500    # screenshot the anim gallery
```

QA URL modes: `/varsity/?quick` jumps straight into a game, `?gallery` shows every animation
