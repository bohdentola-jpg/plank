...
/** Overlay: pin the left arm into a ball-carry tuck. blend 0..1 */
export function tuckOverlay(getBlend) {
  return (pose) => {
    const b = getBlend();
    if (b <= 0) return;
    const mix = (cur, target) => cur + (target - cur) * b;
    pose.shL = [mix(pose.shL[0], 18), mix(pose.shL[1], 26), mix(pose.shL[2], 38)];
    pose.elL = [mix(pose.elL[0], -118), mix(pose.elL[1], -24), mix(pose.elL[2], 0)];
  };
}
